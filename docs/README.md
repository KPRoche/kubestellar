...
It is also important to consider how links will render on GitHub _vs_ kubestellar.io if you are reusing a repository-focused file (a README file, or CONTRIBUTING.md, for instance). To avoid confusion on the website it may be necessary to include the repository file in sections to avoid the internal links, with web-based links inserted between the partical includes. See docs/content/contribution-guidelines/contributing-inc.md for an example of this technique.
